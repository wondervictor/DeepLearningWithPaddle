...
class BeamSearch(object):
    ...
    def __init__(self, inferer, ids_2_word, beam_size=1, max_gen_len=100):
        """
        constructor method.
        ...
        self.ids_2_word = ids_2_word
        print("dictionay len = %d" % (len(self.ids_2_word)))

        try:
            self.eos_id = self.ids_2_word.get('EOS')
            # next(x[0] for x in self.ids_2_word.iteritems()
            #                   if x[1] == "<e>")
            self.unk_id = self.ids_2_word.get('UNK')# next(x[0] for x in self.ids_2_word.iteritems()
             #                  if x[1] == "<unk>")
        except StopIteration:
            print("the word dictionay must contain an ending mark "
...
def generate():
    ...
    last_word = predict_caption_net(image, target, DICT_DIM)

    parameters = paddle.parameters.Parameters.from_tar(gzip.open('params_pass_1.tar.gz'))

    inferer = paddle.inference.Inference(output_layer=last_word, parameters=parameters)
    with open('id2word.pkl', 'rb') as f:
        id2word = pickle.load(f)

    generator = BeamSearch(inferer, id2word, 3, 20)

    image_path = '109202756_b97fcdc62c.jpg'
